...
export default function AdhocSalesPage() {
  ...
  const data = (salesData.sales as any).filter((s: any) => s.saleType === 'Adhoc Sales');

  return (
    <SalesOpsLayout selected="sales" onSelect={(id) => router.push(`/sales-and-operations/${id}`)}>
      <SalesOpsSection title="Adhoc Sales" columns={columns} data={data} />
    </SalesOpsLayout>
  );
}
