...
export default function RouteSalesPage() {
  ...
  const data = (salesData.sales as any).filter((s: any) => s.saleType === 'Route Sales');

  return (
    <SalesOpsLayout selected="sales" onSelect={(id) => router.push(`/sales-and-operations/${id}`)}>
      <SalesOpsSection title="Route Sales" columns={columns} data={data} />
    </SalesOpsLayout>
  );
}
